...
config = Config()
class GamepadModeEvent(ExtremityWalkingEvent):
    """Class representing the walking + extremity triggers gamepad mode. Special mode to allow for walking to perform an action along with holding an
    ...
    def _set_extremity_keys(self) -> None:
        self._walking_keys = config.get_data("events/gamepad_mode_keys/walking_extremities")
        self._button_keys = config.get_data("events/gamepad_mode_keys/button_extremities")
...
class GamepadMode1Event(GamepadModeEvent):
    """Class for Mode 1 of the Gamepad mode. The walking D-pad here is used to change the keybind for walking, so
    ...
    [gestures types]:
        ...
        "extremity_down_button": down button extremity trigger to cause a key down action
        "extremity_right_button": right button extremity trigger to cause a key down action
        "extremity_up_button": up button extremity trigger to cause a key down action
        "extremity_left_button": left button extremity trigger to cause a key down action
    ...
    _button_keys = config.get_data("events/gamepad_mode_keys/button_extremities")
    ...
    def set_up(self) -> None:
        ...
        button_set = {"left_walking", "right_walking", "up_walking", "down_walking"}
        button_keys = config.get_data("events/gamepad_mode_keys/button_extremities")
...
class GamepadMode2Event(GamepadModeEvent):
    """Class for Mode 2 of the Gamepad mode. The triggers in the walking D-pad need to be held with walking together to
    cause an action.
...
class GamepadMode3Event(GamepadMode1Event):
    """Class for Mode 3 of the Gamepad mode. Here, all triggers act as buttons, with walking on the spot fixed
    to the same key.
    ...
    def _set_extremity_keys(self) -> None:
        self._button_keys = config.get_data("events/gamepad_mode_keys/button_extremities") | config.get_data("events/gamepad_mode_keys/walking_extremities")
    ...
    def set_up(self) -> None:
        ...
        button_set = {"left_walking", "right_walking", "down_walking"}
        button_keys = config.get_data("events/gamepad_mode_keys/button_extremities")
        for extremity, key in button_keys.items():
            button_set.add(extremity[10:])
